...
class WB_BSL_Bing
{
    ...
    public static function push_batch_url($urls)
    {
        ...
        $http = wp_remote_post($api,array('headers'=>array('Content-Type'=>'text/json; charset=utf-8'),'body'=>json_encode(array('siteUrl'=>home_url(),'urlList'=>$submit_url))));
        if(is_wp_error($http)){
    ...
    public static function push_url($url)
    {
        ...
        $http = wp_remote_post($api,array('headers'=>array('Content-Type'=>'text/json; charset=utf-8'),'body'=>json_encode(array('siteUrl'=>home_url(),'url'=>$url))));
        if(is_wp_error($http)){
