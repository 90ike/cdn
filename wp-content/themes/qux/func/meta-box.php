...
function um_product_info_callback($post){
	...
	$um_enable_cdkey = get_post_meta($post->ID,'um_enable_cdkey',true);
?>
...
<p style="width:25%;float:left;"><?php _e( '优惠码支持,仅限现金商品 ', 'um' );?>
...
<p style="width:15%;float:left;"><?php _e( '自动发卡 ', 'um' );?>
	<select name="um_enable_cdkey">
		<option value="0" <?php if( $coupon_code_support!==1) echo 'selected="selected"';?>><?php _e( '关闭', 'um' );?></option>
		<option value="1" <?php if( $coupon_code_support===1) echo 'selected="selected"';?>><?php _e( '开启', 'um' );?></option>
	</select>
</p>
...
function um_video_info_callback($post) {
	...
	$video_width = get_post_meta($post->ID,'video_width',true);
	...
	<p style="clear:both;font-weight:bold;border-bottom:1px solid #ddd;padding-bottom:8px;"><?php _e('视频文件、视频地址、视频代码','um'); ?></p>
    <p style="clear:both;"><?php _e( '该参数9.1.7以弃用，尽快将该设置项调用至下方的最新设置', 'um' );?></p>
    <textarea name="dp_video_code" rows="5" class="large-text code"><?php echo $dp_video_file.$dp_video_url.$dp_video_code;?></textarea>
    
    <p style="clear:both;font-weight:bold;border-bottom:1px solid #ddd;padding-bottom:8px;"><?php _e('视频参数','um'); ?></p>
    <p style="width:20%;float:left;"><?php _e( '选择视屏类型', 'um' );?>
	<select name="video_type"><?php echo video_type($video_type); ?></select>
    <p style="width:20%;float:left;"><?php _e( '选择播放器样式', 'um' );?>
	<select name="video_width">
	    <option value="0" <?php if( $video_width!=1) echo 'selected="selected"';?>><?php _e( '文章宽度', 'um' );?></option>
	    <option value="1" <?php if( $video_width==1) echo 'selected="selected"';?>><?php _e( '全宽播放器', 'um' );?></option>
	</select>
	<p style="width:20%;float:left;"><?php _e( '分集显示', 'um' );?>
	<select name="video_list">
	    <option value="0" <?php if( $video_list!=1) echo 'selected="selected"';?>><?php _e( '播放器右方', 'um' );?></option>
		<option value="1" <?php if( $video_list==1) echo 'selected="selected"';?>><?php _e( '播放器下方', 'um' );?></option>
	</select>
	</p>
	<p style="clear:both;font-weight:bold;border-bottom:1px solid #ddd;padding-bottom:8px;"><?php _e('视频地址','um'); ?></p>
    <p style="clear:both;"><?php _e( '根据视频类型填入相应视频连接或ID，每行一条，分集使用$隔开，例如：第一集$http://www.baidu.com', 'um' );?></p>
    <textarea name="video_url" rows="5" class="large-text code"><?php echo $video_url;?></textarea>
...
function um_save_meta_box_data( $post_id ) {
	...
	if ( isset( $_POST['um_enable_cdkey'] ) ) update_post_meta( $post_id, 'um_enable_cdkey', $_POST['um_enable_cdkey'] );
	...
	if ( isset( $_POST['video_type'] ) ) update_post_meta( $post_id, 'video_type', $_POST['video_type'] );
	
	if ( isset( $_POST['video_url'] ) ) update_post_meta( $post_id, 'video_url', $_POST['video_url'] );
	
	if ( isset( $_POST['video_width'] ) ) update_post_meta( $post_id, 'video_width', $_POST['video_width'] );
	
	if ( isset( $_POST['video_list'] ) ) update_post_meta( $post_id, 'video_list', $_POST['video_list'] );
