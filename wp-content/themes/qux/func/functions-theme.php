...
function hide_admin_bar($flag) {
    if(_hui('admin_bar_switch') && is_user_logged_in() && current_user_can( 'manage_options' ) && !wp_is_mobile()) return true;
	return false;
}
...
function _smilies_src($img_src, $img, $siteurl) {
	return THEME_URI . '/img/smilies/' . $img;
}
...
function _jsloader($arr) {
	foreach ($arr as $item) {
		wp_enqueue_script('_' . $item, THEME_URI . '/js/' . $item . '.js', array(), THEME_VERSION, true);
	}
}

function _get_default_avatar(){
	return THEME_URI . '/img/avatar-default.png';
}
...
function _get_post_thumbnail($width = 220, $height = 150, $size = 'post-thumbnail', $class = 'thumb') {
	...
	if( $r_src ){
		...
		if( _hui('thumbnail_src') ){
    		return sprintf('<img data-src="%s" alt="%s" src="%s" class="lazy '.$class.'">',$r_src, $post->post_title._get_delimiter().get_bloginfo('name'),THEME_URI.'/img/thumbnail.png');
		}else{
    	...
    	if(get_post_type() == 'store'){
    	    ...
    	    return sprintf('<img data-thumb="default" src="%s" class="'.$class.'">', THEME_URI.'/img/thumbnail.png');	
    	}
    }
}
	...
	$post_thumbnail_src = '';
   	...
   	if( has_post_thumbnail( $post->ID ) ){    //如果有特色缩略图，则输出缩略图地址
        ...
        if(!empty($matches[1][0])){
			...
			$post_thumbnail_src = THEME_URI.'/img/rand/'.rand(1,15).'.jpg';
        }
    }
...
function _get_taxonomy_tags($args = array( 'taxonomy' => 'category','taxonomy_tag' => 'post_tag', 'id' => '')) {
    global $wpdb;
    $tags = $wpdb->get_results
        ...
        WHERE
            t1.taxonomy = '".$args['taxonomy']."' AND p1.post_status = 'publish' AND terms1.term_id IN (" . $args['id'] . ") AND
            t2.taxonomy = '".$args['taxonomy_tag']."' AND p2.post_status = 'publish'
            AND p1.ID = p2.ID
    ...
    if ($tags) {
        foreach ($tags as $tag) {
            $mytag[$count] = get_term_by('id', $tag->tag_id, $args['taxonomy_tag']);
            $count++;
        }
    } else {
        $mytag = null;
...
function qux_get_term_parent($term, $taxonomy){
    	
    $parent  = get_term_by( 'id', $term, $taxonomy);
    while ($parent->parent){
