...
class Avatar {
	...
	const IMAGES_PATH = THEME_URI . '/img/avatar'; // 图片目录

	// 默认配置:
	private $use_gravatar; // 显示Gravatar头像
	private $gravatar_uri;  // Gravatar头像获取地址
	private $letter_index = 0;  // 0:首字母;1:第二字母;-1:最后一个字母等。
	private $images_format = 'png';  // 文件格式
	private $image_unknown = 'mystery'; // 未知头像名称，

    // 创建函数
	public function __construct($gravatar_uri = 'https://secure.gravatar.com/avatar/', $use_gravatar = false) {
	    
	    $this->gravatar_uri = $gravatar_uri;
	    $this->use_gravatar = $use_gravatar;

		/* --------------- WP 钩子 --------------- */
		// add filter to get_avatar:
		add_filter('get_avatar', array($this, 'set_comment_avatar'), 10, 6);
		...
		if (!is_admin()){
			...
			if ($pagenow == 'options-discussion.php'){
				remove_filter('get_avatar', array($this, 'set_comment_avatar'), 10);
			}
		}
	}
	...
	public function admin_bar_menu_action(){ // change avatar in the userbar at the top
		...
		add_filter('get_avatar', array($this, 'set_userbar_avatar'), 10, 6);
	...
	private function generate_first_letter_uri($name, $size){
			...
			if (!empty($charset_flag)){ // if charset_flag is not empty, i.e. flag has been set to latin, number or cyrillic...
				switch ($charset_flag){ // run through various options to determine the actual filename for the letter avatar
					...
					case 'latin':
						...
						break;
					case 'cyrillic':
					    if($str = iconv('UTF-8', 'UCS-4LE', $file_name_mb)){
					    	$temp_array = unpack('V', $str); // beautiful one-liner by @bobince from SO - http://stackoverflow.com/a/27444149/4848918
		...
		// 判断avatar大小选择不同的图片目录:
		if ($size <= 96) $custom_avatar_size = '96';
		else if ($size > 96) $custom_avatar_size = '128';

		$avatar_uri = self::IMAGES_PATH . '/' . $custom_avatar_size . '/' . $file_name . '.' . $this->images_format;
...
$gravatar_uri = _hui('gravatar_url') == 'ssl' ? 'https://secure.gravatar.com/avatar/' : 'https://cdn.v2ex.com/gravatar/'; // gravatar获取url
$_avatar = new Avatar($gravatar_uri, _hui('gravatar_bom'));