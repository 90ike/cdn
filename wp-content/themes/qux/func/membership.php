...
//会员标识
function um_member_icon($uid, $link = false){
    if($uid == 0 || $uid == null){
       return false;
    }
	$uid = (int)$uid;
	$type = getUserMemberType($uid);
	$link = $link ? 'href="'.add_query_arg('tab', 'membership', esc_url( get_author_posts_url( $uid ) )).'"' : '';
	//0代表已过期 1代表月费会员 2代表季费会员 3代表年费会员 FALSE代表未开通过会员
    if($type==4)$icon = '<a class="ico forever_member" '.$link.' title="终身会员"></a>';
	elseif($type==3)$icon = '<a class="ico annual_member" '.$link.' title="年费会员"></a>';
	elseif($type==2)$icon = '<a class="ico quarter_member" '.$link.' title="季度会员"></a>';
	elseif($type==1)$icon = '<a class="ico month_member" '.$link.' title="月费会员"></a>';
	elseif($type==0)$icon = '<a class="ico expired_member" '.$link.' title="你还不是会员或会员已过期"></a>';
	else $icon = '<a class="ico expired_member" '.$link.' title="你还不是会员"></a>';
	return $icon;
}
