...
function get_author_class($user_email, $user_id){  
    ...
    if(!empty(get_userdata($user_id)->roles) && in_array('administrator',get_userdata($user_id)->roles)) { 
    	...
    	if($author_count>=1 && $author_count<5)  
...
function _get_flag($ip) {
	...
	$output = str_replace("%IMAGE_BASE%", THEME_URI .'/img/flags', $output);
	
	return $output;
}

function _get_flag_without_template($ip, $show_image = true, $show_text = true, $before = '', $after = '') {
	if($ip == '127.0.0.1'){
		$code = 'wordpress';
		$name = 'Localhost';
	}else{
...
function _get_comment_flag() {
	$ip = get_comment_author_IP();
	return _get_flag($ip);
}

function _get_comment_flag_without_template() {
	$ip = get_comment_author_IP();
	return _get_flag_without_template($ip);
}

	
/* 浏览器 */
$CID_image_url = THEME_URI ."/img/browsers/";
$CID_width_height = "14px";

function _windows_detect_os($ua) {
	...
	elseif (preg_match('/Mac_PowerPC/i', $ua)) {
		...
		list($os_name, $os_code, $os_ver, $pda_name, $pda_code, $pda_ver) = _pda_detect_os($ua);
		$os_name = "Windows";
		$os_code = "windows";
		$os_ver = "CE";
...
function _detect_browser($ua) {
	...
	if (preg_match('#MovableType[ /]([a-zA-Z0-9.]+)#i', $ua, $matches)) {
		...
		$browser_ver = $matches[1];
		list($os_name, $os_code, $os_ver) = _unix_detect_os($ua);
		...
		if (preg_match('/Windows/i', $ua)) {
			list($os_name, $os_code, $os_ver) = _windows_detect_os($ua);
		} else {
			list($os_name, $os_code, $os_ver) = _unix_detect_os($ua);
		}
		...
		list($os_name, $os_code, $os_ver) = _unix_detect_os($ua);
		if (!$os_name) {
			list($os_name, $os_code, $os_ver) = _pda_detect_os($ua);
		}
	...
	// mice
		...
		elseif ( strpos($browser_ver, '10') !== false)
			...
			$browser_code = 'ie';
		list($os_name, $os_code, $os_ver, $pda_name, $pda_code, $pda_ver) = _windows_detect_os($ua);
...
function _friendly_string($browser_name = '', $browser_code = '', $browser_ver = '', $os_name = '', $os_code = '', $os_ver = '', $pda_name= '', $pda_code = '', $pda_ver = '') {
	...
	$output = str_replace("%IMAGE_BASE%", THEME_URI ."/img/browsers", $output);
...
function _friendly_string_without_template($browser_name = '', $browser_code = '', $browser_ver = '', $os_name = '', $os_code = '', $os_ver = '', $pda_name= '', $pda_code = '', $pda_ver = '', $show_image = true, $show_text = true, $between = '', $before = '', $after = '') {
...
function _browser_string($ua) {
	list ($browser_name, $browser_code, $browser_ver, $os_name, $os_code, $os_ver, $pda_name, $pda_code, $pda_ver) = _detect_browser($ua);
	$string = _friendly_string($browser_name, $browser_code, $browser_ver, $os_name, $os_code, $os_ver, $pda_name, $pda_code, $pda_ver);
	/*if (!$string) {
...
function _browser_string_without_template($ua, $show_image = true, $show_text = true, $between = '', $before = '', $after = '') {
	list ($browser_name, $browser_code, $browser_ver, $os_name, $os_code, $os_ver, $pda_name, $pda_code, $pda_ver) = _detect_browser($ua);
	$string = _friendly_string_without_template($browser_name, $browser_code, $browser_ver, $os_name, $os_code, $os_ver, $pda_name, $pda_code, $pda_ver, $show_image, $show_text, $between, $before, $after);
	/*if (!$string) {
		$string = "Unknown browser";
	}*/
	return $string;
...
function _get_comment_browser() {
	global $comment;
	if (!$comment->comment_agent) return;
	$string = _browser_string($comment->comment_agent);
	return $string;
}

function _get_comment_browser_without_template() {
	global $comment;
	if (!$comment->comment_agent) return;
	$string = _browser_string_without_template($comment->comment_agent);
	return $string;
}
