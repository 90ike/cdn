	...
	<?php 
	    ...
	    if(_hui('home_topic_cat') && is_array(_hui('home_topic_cat'))){
	        foreach (_hui('home_topic_cat') as $cat){
	            $c = get_term($cat, 'tcat');
	            if($c){
	            $html .= '<div class="zhuanti-list-item ht_custom_grid_1_4"><a class="thumbnail-link" href="'.get_term_link($c->term_id).'"><div class="thumbnail-wrap"><img src="' . THEME_URI . '/func/timthumb.php?src='. topic_taxonomy_image_url($c->term_id).'&w=186&h=120" alt="'.$c->name.'" class="thumb"></div></a><h3 class="zhuanti-title"><a href="'.get_term_link($c->term_id).'">'. $c->name .'</a></h3></div>';
	            }
	        }
	    }
