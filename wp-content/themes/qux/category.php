...
<?php if( !_hui('is_filter_bar') && (mo_is_minicat() || $widthfull)){ ?>
...
<?php } ?>
<?php _moloader('mo_post_filter', false); ?>
		...
		<div class="content">
			...
			<?php 
				if( mo_is_minicat() ){
					...
					if(!$widthfull && !_hui('is_filter_bar')) echo '<div class="pagetitle'.$layout.'"><h1>', single_cat_title(), $pagedtext.'</h1>'.'<div class="catleader-desc">'.$description.'</div></div>';
					if(have_posts()){
