...
function mo_comments_list($comment, $args, $depth) {
    ...
    global $commentcount, $wpdb, $post,$current_user;
    ...
    echo '<div class="comt-main" id="div-comment-'.get_comment_ID().'">';
        ...
        echo '<div class="comt-meta"><span class="comt-author">';
			
        if ($user_id==1){
			echo '<span class="super-author">管理员</span>';
		}
        ...
        if (intval($vip_type) == 31) {
            $name = '<span class="comment-vp1">月费会员</span> ';
        } elseif (intval($vip_type) == 365) {
            $name = '<span class="comment-vp2">年费会员</span> ';
        } elseif (intval($vip_type) == 3600) {
            $name = '<span class="comment-vp3">终身会员</span> ';
        }
    } else {
        $name = null;
