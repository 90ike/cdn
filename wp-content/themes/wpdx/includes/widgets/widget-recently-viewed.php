<?php
add_action('widgets_init', 'cmp_recent_viewed_posts');
function cmp_recent_viewed_posts() {
    register_widget("recent_viewed_posts");
}
class recent_viewed_posts extends WP_Widget {
        ...
        <p class="typeholder">
            ...
            if(isset($_COOKIE['cm_recent_posts']) && $_COOKIE['cm_recent_posts']!=''){
                ...
                if (count($cm_cookie_posts) > 0) {
                    ...
                    foreach ( $cm_cookie_posts as $postId ) {
                        ...
                        if ($post_exists && $cm_post && $cm_post->ID != $currentPostId && in_array($cm_post->post_type,$selected_posttypes) ) {
                            $count++;
                            echo '<li><a href="'.get_permalink( $cm_post->ID ).'" title="'.get_the_title($cm_post->ID).'">'.get_the_title($cm_post->ID).'</a></li>';
                        } 
                    } 
                    echo '</ul>';
                    echo $after_widget;
