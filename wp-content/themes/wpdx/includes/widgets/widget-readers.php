<?php
//widget readers
add_action('widgets_init', 'cmp_readers');
function cmp_readers() {
...
class readers extends WP_Widget {
	public function __construct() {
        parent::__construct(
            ...
            array('classname' => 'widget_readers' ,'description' => __( 'Display avatar of the most active readers.', 'wpdx' )),
	...
	public function widget($args, $instance) {
		...
		echo '<ul>';
		echo cmp_most_comments_readers( $outer, $timer, $limit );
		echo '</ul>';
...
/*
 * 读者墙
 * cmp_most_comments_readers( $outer='name', $timer='3', $limit='14' );
 * $outer 不显示某人
 * $timer 几个月时间内
 * $limit 显示条数
*/
function cmp_most_comments_readers($out,$tim,$lim){
  global $wpdb;
  ...
  foreach ($readers as $reader) {
    ...
    echo '<li class="avatar-'.$i.'"><a rel="nofollow" target="_blank" href="'. $c_url . '" title="' . $reader->comment_author . ' + '. $reader->cnt . '" >'.get_avatar($reader->comment_author_email,60).'</a></li>';
    $i++;
  }
}