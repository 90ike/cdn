 ...
 * @Last Modified time: 2019-02-16 15:30:45
 ...
 * @since 1.0
 */
if( cmp_get_option('login_url') && !defined('CMP_REMOVE_LOGIN_URL') && cmp_get_option('enable_cmpuser')){
    ...
    function cmpuser_login_page( $login_url, $redirect, $force_reauth ) {
        ...
        if(is_ssl()){
            $login_page = str_replace( 'http://', 'https://', $login_page );
        }
...
if(cmp_get_option('register_url') && cmp_get_option('enable_cmpuser')){
    add_filter( 'register_url', 'cmpuser_register_page' );
    function cmpuser_register_page( $register_url ) {
        $register_url = esc_url(cmp_get_option('register_url'));
        if(is_ssl()){
...
if(cmp_get_option('password_url') && cmp_get_option('enable_cmpuser')){
    ...
    function cmpuser_lost_password_page( $lostpassword_url, $redirect ) {
        ...
        if(is_ssl()){
            $lostpassword_url = str_replace( 'http://', 'https://', $lostpassword_url );
        }
...
function show_cmpuser_login_form($atts) {
    ...
    if ( is_user_logged_in() ) {
        ...
        require(get_template_directory() . '/cmpuser/templates/login-preview.php' );

    } else {
        // show login form
        require(get_template_directory() . '/cmpuser/templates/login-form.php' );
    }
...
if(!function_exists('cmpuser_scripts')){
    include_once( get_template_directory() . '/cmpuser/templates/new-profile-edit-form.php' );
}
...
function show_cmpuser_register_form($atts) {
    ...
    if ( !is_user_logged_in() ) {
        require(get_template_directory() . '/cmpuser/templates/register-form.php' );
    } else {
...
function show_cmpuser_reset_password_form($atts) {
    ...
    if ( !is_user_logged_in() ) {
        if ( isset( $_GET['pass'] ) ) {
            ...
            require(get_template_directory() . '/cmpuser/templates/show-new-password.php' );
        } else{
            require(get_template_directory() . '/cmpuser/templates/reset-password-form.php' );
        }
    } else {
                        ...
                        if( $emailnotificationuser ) {
                            if(!$emailnotificationcontent || $emailnotificationcontent == ''){
                                $emailnotificationcontent = sprintf( __( "Thanks for registering on our website, the following is your account information:<br/><br/>
                                    ...
                                    Password: %s <br/><br/>
                                    Email: %s <br/><br/>
                                    You can login from: <a href='%s' target='_blank'>%s</a> <br/><br/>
                                    ", 'wpdx' ), $username, $pass1, $email, cmp_get_option('login_url'),cmp_get_option('login_url') );
                            }else{
                            ...
                            $blog_title = get_bloginfo();
                            $subject = "[$blog_title] " . __( 'Thanks for registering on our website', 'wpdx' );
                            add_filter( 'wp_mail_content_type', 'cmpuser_set_html_content_type' );
...
/**
 * [cmpuser_get_user_list description]
 ...
 * @since 3.7
 */
function cmpuser_get_user_list($role = '',$orderby = '', $order='',$perpage = ''){
    $orderby = $orderby ? $orderby : 'post_count';
    ...
    $totalPages = intval( $total/ $perpage ) + 1;


    // User Loop
    if ( ! empty( $user_query->results ) ) {
        foreach ( $user_query->results as $user ) {
            cmp_author_box(true,true,true,$user->ID);
        }
...
/**
 * [cmpuser_user_list description]
 * @param  [type] $atts [description]
 * @return [type]       [description]
 * @since 3.7
 */
function cmpuser_user_list($atts){
    $atts = shortcode_atts(
        array(
            'role' => '',
            'perpage' => '',
    ...
    ob_start();
    $output = cmpuser_get_user_list($atts['role'],$atts['orderby'],$atts['order'],$atts['perpage']);
    return $output = ob_get_clean();
}
add_shortcode('cmpuser-user-list', 'cmpuser_user_list');