...
function cmp_return_array(){
    return array('');
}
function cmp_return_false(){
    return false;
}
...
switch(cmp_get_option('editor_style')){
    case 'simple':
        ...
        add_filter( 'teeny_mce_buttons', 'cmp_return_array' , 50 );
        break;
    case 'rich':
        $teeny = false;
        $show_quicktags = true;
    ...
    case 'html':
        $teeny = true;
        $show_quicktags = true;
        add_filter ( 'user_can_richedit' , 'cmp_return_false' , 50 );
        break;
}
...
<form id="frontend_post_form" class="cmpuser_frontend_post_form bordered" method="post" action="<?php echo admin_url('admin-ajax.php'); ?>" enctype="multipart/form-data">
    ...
    <?php if (cmp_get_option('login_link') && !is_user_logged_in()) { ?>
        <a href="<?php echo wp_login_url( get_permalink() ); ?>" title="<?php _e('Login', 'wpdx' ); ?>"><?php _e('Login', 'wpdx' ); ?></a>
    <?php } ?>
    <div id="field-wrapper" class="cmpuser-field">
        <label for="cmpuser_frontend_post_titel"><?php _e('Title', 'wpdx' ); ?></label>
        ...
        <?php if (cmp_get_option('show_excerpt')) { ?>
            <label for="cmpuser_frontend_post_excerpt"><?php _e('Excerpt', 'wpdx' ); ?></label>
        ...
        <?php } ?>
        <label for="cmpufpcontent"><?php _e('Content ', 'wpdx' ); ?></label>
        <?php
        $settings = array(
        ...
        if ( is_user_logged_in() || cmp_get_option('guest_cat_select') ){
            ...
            switch(cmp_get_option('display_categories')){
                ...
                case 'list':
                    ...
                    <label for="select_post_category"><?php _e('Select a Category', 'wpdx' ); ?></label>
                ...
                case 'check':
                    ...
                    <label for="cmpuser_frontend_post_cat_checklist"><?php _e('Category', 'wpdx') ; ?></label>
        ...
        if (cmp_get_option('allow_new_category') && $verified_user['cmp_can_manage_categories']) { ?>
            <label for="cmpuser_frontend_post_new_category"><?php _e('New category', 'wpdx' ); ?></label>
        ...
        if (cmp_get_option('allow_tags')) { ?>
            <label for="cmpuser_frontend_post_tags"><?php _e('Tags (comma-separated)', 'wpdx') ; ?></label>
    ...
    <button type="submit" class="send-button theme-button" id="submit"><?php _e('Submit', 'wpdx' ); ?></button>
