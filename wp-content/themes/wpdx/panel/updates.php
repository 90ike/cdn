...
if( is_admin() ){
	$theme_options = get_option( 'cmp_options' );
	if( get_option('cmp_'.'wpdx'.'_active') < 3.0 ){
		...
		update_option( 'cmp_options' , $theme_options );
		update_option( 'cmp_'.'wpdx'.'_active' , '3.0' );
		echo '<script>location.reload();</script>';
		die;
	}

	if( get_option('cmp_'.'wpdx'.'_active') < 3.2 ){

		$theme_options['enable_frontend_post'] = true;
		$theme_options['enable_cmpuser']  = true;

		update_option( 'cmp_options' , $theme_options );
		update_option( 'cmp_'.'wpdx'.'_active' , '3.2' );
		echo '<script>location.reload();</script>';
		die;
	}

	if( get_option('cmp_'.'wpdx'.'_active') < 3.3 ){
		...
		update_option( 'cmp_options' , $theme_options );
		update_option( 'cmp_'.'wpdx'.'_active' , '3.3' );
		echo '<script>location.reload();</script>';
		die;
	}
//For Debugging 
//update_option( 'cmp_'.'wpdx'.'_active' , '3.3' );
//delete_option( 'cmp_'.'wpdx'.'_active');
}
?>