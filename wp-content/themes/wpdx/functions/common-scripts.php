...
function cmp_register() {
  ...
  if(cmp_get_option('jquery_cdn') =='default'){
    ...
    if(cmp_get_option('jquery_cdn') == 'jquery') {
      ...
      $jquery_cdn = '//cdn.staticfile.org/jquery/1.10.2/jquery.min.js';
      $jquery_migrate_cdn = '//cdn.staticfile.org/jquery-migrate/1.2.1/jquery-migrate.min.js';
    }
  ...
  wp_register_script( 'base-js', get_template_directory_uri() . '/assets/js/base.js', array('jquery'), THEME_VER, true );
  wp_enqueue_script( 'base-js' );
  if ( is_home() || is_active_widget( '', '', 'slider-widget' ) ) {
    wp_register_script( 'BxSlider', get_template_directory_uri() . '/assets/js/BxSlider.min.js', array('jquery'), THEME_VER, true );
    wp_enqueue_script( 'BxSlider' );
  }

  if( cmp_get_option('share_post')||cmp_get_option('edd_share_post')){
    wp_register_script( 'share-js', get_template_directory_uri() . '/assets/js/jquery.share.min.js', array('jquery'), THEME_VER , true );
    wp_enqueue_script( 'share-js' );
  }

  if(cmp_get_option('blog_pagination_type') == 'ajax' || cmp_get_option('archive_pagination_type') == 'ajax' ){
    wp_register_script( 'ajax-paged', get_template_directory_uri() . '/assets/js/jquery-ias.js', array('jquery'), THEME_VER , true);
  ...
  if(cmp_get_option('font_awesome_cdn') == 'qiniu'){
    $font_awesome_cdn = '//cdn.staticfile.org/font-awesome/4.7.0/css/font-awesome.min.css';
  }elseif(cmp_get_option('font_awesome_cdn') == 'bootcss'){
    $font_awesome_cdn = '//cdn.bootcss.com/font-awesome/4.7.0/css/font-awesome.min.css';
  }elseif(cmp_get_option('font_awesome_cdn') == 'cloudflare'){
    $font_awesome_cdn = '//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css';
  }else{
    $font_awesome_cdn = get_template_directory_uri() . '/assets/css/font-awesome.min.css';
  }
  wp_register_style( 'font-awesome', $font_awesome_cdn ,'','' );
  ...
  if( class_exists('Front_End_Pm') && is_page() ){
    wp_enqueue_style( 'wpdx-fep', get_template_directory_uri() . '/assets/css/fep.css','',THEME_VER );
  }

  if(is_singular() && comments_open()){
    wp_enqueue_script( 'comments-ajax', get_template_directory_uri() . '/comments-ajax.js', array('jquery'), THEME_VER, true);
  ...
  if(is_singular() && cmp_get_option('smilies')){
    wp_enqueue_script( 'smilies', get_template_directory_uri() . '/assets/js/smilies.js', '', THEME_VER, true);
  }
  ...
  if ( comments_open() && is_singular() && $post->post_type != 'dwqa-question' && $post->post_type != 'dwqa-answer' && !is_page('new-post' ) && !is_page('edit' ) && cmp_get_option('comment_quicktags')) {
    wp_enqueue_script( 'my-quicktags', get_template_directory_uri() . '/assets/js/my-quicktags.js', array('quicktags','jquery'), THEME_VER, true);
  }
  ...
  if( cmp_get_option('right_rolling') ){
    ...
    $var = 'var right_1 = '.$r_1.',right_2 = '.$r_2;
    wp_enqueue_script( 'right_rolling', get_template_directory_uri() . '/assets/js/post.js', array('jquery'), THEME_VER, true);
    wp_add_inline_script( 'right_rolling', $var, 'before' );
  }

  if( cmp_get_option('show_weibo')){
    wp_enqueue_script( 'sina-weibo-api', '//tjs.sjs.sinajs.cn/open/api/js/wb.js', '', '', true);
  }

  if( class_exists('WeDocs') ){
     wp_enqueue_style( 'wpkj-wedocs', get_template_directory_uri() . '/assets/css/wedocs.css', array('wedocs-styles'), THEME_VER );
  }
}
/*-----------------------------------------------------------------------------------*/
# Cmp Wp Head
