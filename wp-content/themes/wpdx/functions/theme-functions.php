 ...
 * disable from 4.3
 */
// add_filter('nav_menu_css_class', 'cmp_css_attributes_filter', 100, 1);
// add_filter('nav_menu_item_id', 'cmp_css_attributes_filter', 100, 1);
// add_filter('page_css_class', 'cmp_css_attributes_filter', 100, 1);
function my_css_attributes_filter($var) {
    return is_array($var) ? array_intersect($var, array('page_item','wd-state-open','wd-state-closed','current-menu-item','current-post-ancestor','current-menu-ancestor','current-menu-parent','display-none','children','current_page_parent','current_page_item','current_page_ancestor','page_item_has_children')) : '';
}
/*-----------------------------------------------------------------------------------*/
# Get Theme Options
/*-----------------------------------------------------------------------------------*/
 ...
 * Since 2.4
 */
require_once(get_template_directory() . '/functions/otf_regen_thumbs.php');
...
/*-----------------------------------------------------------------------------------*/
function cmp_author_box($avatar = true , $social = true , $name = false , $user_id = false){
    if(empty($user_id)) $user_id = get_the_author_meta( 'ID' );
    if( $avatar ) : ?>
    <div class="author-avatar">
        <?php echo get_avatar( get_the_author_meta( 'user_email' , $user_id ), 64 ); ?>
    </div>
<?php endif; ?>
<div class="author-description">
    <?php if( !empty( $name ) ): ?>
        <p class="user-name"><a href="<?php echo get_author_posts_url( $user_id ); ?>"><?php echo get_the_author_meta( 'display_name', $user_id) ?> </a></p>
    <?php endif; ?>
    <p><?php if(get_the_author_meta( 'description' )){
        echo get_the_author_meta( 'description' );
    }else{
        $current_user = wp_get_current_user();
        if(cmp_get_option('profile_url') && $current_user->ID == $user_id){
...
<?php  if( $social ) :  ?>
    <ul class="author-social follows nb">
        <?php  if( !is_author() ) :
        $userlinks = get_author_posts_url( $user_id );
        ?>
        <li class="archive">
            <a target="_blank" href="<?php echo $userlinks; ?>" title="<?php echo sprintf( __( "Read more articles of %s", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?>"><?php echo sprintf( __( "Read more articles of %s", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?></a>
        </li>
    <?php endif; ?>
    <?php if ( get_the_author_meta( 'url', $user_id ) ) : ?>
        <li class="website">
            <a target="_blank" rel="nofollow" href="<?php the_author_meta( 'url', $user_id ); ?>" title="<?php echo sprintf( __( "Visit %s's site", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?>"><?php echo sprintf( __( "Visit %s's site", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?></a>
        </li>
    <?php endif ?>
    <?php if ( get_the_author_meta( 'qq', $user_id ) ) : ?>
        <li class="qq">
            <a target="_blank" rel="nofollow" href="http://wpa.qq.com/msgrd?v=3&amp;site=qq&amp;menu=yes&amp;uin=<?php the_author_meta( 'qq', $user_id ); ?>" title="<?php echo sprintf( __( "Contact %s by QQ", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?>"><?php echo sprintf( __( "Contact %s by QQ", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?></a>
        </li>
    <?php endif ?>
    <?php if(class_exists("fep_main_class")){ ?>
    <li class="email">
        <a target="_blank" rel="nofollow" href="<?php echo get_home_url(); ?>/user/pm?fepaction=newmessage&amp;to=<?php echo get_the_author_meta( 'user_login', $user_id ); ?>" title="<?php echo sprintf( __( "Contact %s by Private Messages", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?>"><?php echo sprintf( __( "Contact %s by Private Messages", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?></a>
    </li>
    <?php } elseif(get_the_author_meta( 'qm_mailme', $user_id )) { ?>
    <li class="email">
        <a target="_blank" rel="nofollow" href="<?php the_author_meta( 'qm_mailme', $user_id ); ?>" title="<?php echo sprintf( __( "Contact %s by Email", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?>"><?php echo sprintf( __( "Contact %s by Email", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?></a>
    </li>
    <?php } ?>
    <?php if ( get_the_author_meta( 'sina_weibo', $user_id ) ) : ?>
        <li class="sina_weibo">
            <a target="_blank" rel="nofollow" href="<?php the_author_meta( 'sina_weibo', $user_id ); ?>" title="<?php echo sprintf( __( "Follow %s on Sina Weibo", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?>"><?php echo sprintf( __( "Follow %s on Sina Weibo", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?></a>
        </li>
    <?php endif ?>
    <?php if ( get_the_author_meta( 'twitter', $user_id ) ) : ?>
        <li class="twitter">
            <a target="_blank" rel="nofollow" href="<?php the_author_meta( 'twitter', $user_id ); ?>" title="<?php echo sprintf( __( "Follow %s on Twitter", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?>"><?php echo sprintf( __( "Follow %s on Twitter", 'wpdx' ), get_the_author_meta( 'display_name', $user_id )); ?></a>
        </li>
    <?php endif ?>
</ul>
</div>
...
function cm_comment($comment, $args, $depth) {
    ...
    if($comorder == 'asc'){
        ...
        if(!$commentcount) {
            $comments = $wpdb->get_results("SELECT * FROM $wpdb->comments WHERE comment_post_ID = $post->ID AND comment_approved = '1' AND !comment_parent");
            $cnt = count($comments);
...
function cmp_get_time(){
    ...
    if( cmp_get_option( 'time_format' ) == 'none' ){
        ...
        $t_time    = get_the_time( __( 'Y/m/d g:i:s a', 'wpdx' ), $post );
        $time      = get_post_timestamp( $post );
        $time_diff = time() - $time;

        if ( $time && $time_diff > 0 && $time_diff < YEAR_IN_SECONDS ) {
            /* translators: %s: Human-readable time difference. */
            $since = sprintf( __( '%s ago', 'wpdx' ), human_time_diff( $time ) );
        } else {
...
/**
 * [cmp_get_user_latest_posts description]
 * @param  [type]  $user_id [description]
 * @param  integer $number  [description]
 * @return [type]           [description]
 */
function cmp_get_user_latest_posts($user_id,$number=4){
    $args = array(
...
if( !function_exists('cmp_wedocs_search_form') ):
    /**
     * [cmp_wedocs_search_form description]
     * @return [type] [description]
     * @since 4.0
     */
    function cmp_wedocs_search_form() {
        
        $dropdown_args = array(
            'post_type'         => 'docs',
            'echo'              => 0,
            'depth'             => 1,
            'show_option_none'  => __( 'All Docs', 'wpdx' ),
        ...
        $form = '<form role="search" method="get" class="search-form wedocs-search-form" action="' . esc_url( home_url( '/' ) ) . '">
            <div class="wedocs-search-input">
                ...
                <input type="search" class="search-field" placeholder="' . esc_attr_x( 'Documentation Search &hellip;', 'placeholder', 'wpdx' ) . '" value="' . get_search_query() . '" name="s" title="' . esc_attr_x( 'Search for:', 'label', 'wpdx' ) . '" />
                <input type="hidden" name="post_type" value="docs" />
            </div>
            <div class="wedocs-search-in">
            ' . wp_dropdown_pages( $dropdown_args ) . '
            </div>
            <input type="submit" class="search-submit" value="'. esc_attr_x( 'Search', 'submit button', 'wpdx' ) .'" />
        </form>';
...
if ( ! function_exists( 'cmp_custom_archive_pre_get_posts' ) ) :
    /**
     * Custom archive pre_get_posts
     * @param  [type] $query [description]
     * @return [type]        [description]
     * @since 4.1
     */
   function cmp_custom_archive_pre_get_posts( $query ) {
       ...
       if ( !is_admin() && $query->is_main_query() ) {
            ...
            if(is_home()){
                ...
                if(cmp_get_option( 'exclude_posts' )) {
                    $exclude_posts = explode(',', cmp_get_option( 'exclude_posts' ));
                    $query->set( 'post__not_in', $exclude_posts );
                }

                if(cmp_get_option( 'exclude_categories')) {
                    $exclude_categories = explode(',', cmp_get_option( 'exclude_categories' ));
                ...
                if(cmp_get_option( 'blog_posts_number' )) {
                    $query->set( 'posts_per_page', cmp_get_option( 'blog_posts_number' ) );
                }
                
                if(cmp_get_option('post_types_for_home')){
                    $query->set( 'post_type', cmp_get_option('post_types_for_home') );
                }
            } elseif(is_archive()){

                if( cmp_get_option('archive_mobile') && cmp_is_mobile()){
                    $query->set( 'posts_per_page', cmp_get_option('archive_mobile_number') );
                } elseif ( is_category(explode(',', cmp_get_option('big_thumb') )) ){
                    $query->set( 'posts_per_page', cmp_get_option('big_thumb_number') );
                } elseif ( is_category(explode(',', cmp_get_option('big_thumb') )) ){
                    $query->set( 'posts_per_page', cmp_get_option('big_thumb_number') );
                } elseif ( is_category(explode(',', cmp_get_option('row_thumb') )) ){
                    $query->set( 'posts_per_page', cmp_get_option('row_thumb_number') );
                } elseif ( is_category(explode(',', cmp_get_option('original_image') )) ){
                    $query->set( 'posts_per_page', cmp_get_option('original_image_number') );
                } elseif ( is_category(explode(',', cmp_get_option('small_thumb') )) ){
                    $query->set( 'posts_per_page', cmp_get_option('small_thumb_number') );
                } elseif ( is_category(explode(',', cmp_get_option('simple_title') )) ){
                    $query->set( 'posts_per_page', cmp_get_option('simple_title_number') );
                } elseif ( is_category(explode(',', cmp_get_option('grid_thumb') )) ){
                    $query->set( 'posts_per_page', cmp_get_option('grid_thumb_number') );
                } else {
                    $query->set( 'posts_per_page', cmp_get_option('default_number') );
                }
            }
...
add_action( 'pre_get_posts', 'cmp_custom_archive_pre_get_posts', 99, 1 );

/**
 * Disable wp 5.5 lazy loding
 * since 4.3
*/
