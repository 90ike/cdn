...
class Nav_Menu_Roles {
    ...
    */
    public function __clone() {
        _doing_it_wrong( __FUNCTION__, __( 'Cloning this object is forbidden.' , 'wpdx' ), '1.5' );
    }
    ...
    */
    public function __wakeup() {
        _doing_it_wrong( __FUNCTION__, __( 'Unserializing instances of this class is forbidden.' , 'wpdx' ), '1.5' );
    }
    ...
    */
    public function register_importer(){
        ...
        if ( defined( 'WP_LOAD_IMPORTERS' ) ) {
            ...
            include_once( get_template_directory() . '/functions/class.Nav_Menu_Roles_Import.php');
            // Register the custom importer we've created.
            $roles_import = new Nav_Menu_Roles_Import();

            register_importer( 'nav_menu_roles', __( 'Nav Menu Roles', 'wpdx' ), sprintf( __( 'Import %snav menu roles%s and other menu item meta skipped by the default importer', 'wpdx' ), '<strong>', '</strong>' ), array( $roles_import, 'dispatch' ) );
    ...
    public function edit_nav_menu_walker( $walker ) {
        ...
        if( ! class_exists( 'Walker_Nav_Menu_Edit_Roles' ) ){
            ...
            if ( version_compare( $wp_version, '4.7', '>=' ) ){
                include_once( get_template_directory() . '/functions/class.Walker_Nav_Menu_Edit_Roles_4.7.php');
            } else {
                include_once( get_template_directory() . '/functions/class.Walker_Nav_Menu_Edit_Roles.php');
            }
        }
    ...
    */
	...
	?>
		...
		<fieldset class="field-nav_menu_role nav_menu_logged_in_out_field description-wide" style="margin: 5px 0;">
			<legend class="description"><?php esc_html_e( 'Display Mode', 'wpdx' ); ?></legend>
			...
			<label for="nav_menu_logged_in-for-<?php echo esc_attr( $item->ID ); ?>" style="<?php echo esc_attr( $float ); ?> width: 35%;">
				...
				<?php esc_html_e( 'Logged In Users', 'wpdx' ); ?>   
			</label>
			...
			<label for="nav_menu_logged_out-for-<?php echo esc_attr( $item->ID ); ?>" style="<?php echo esc_attr( $float ); ?> width: 35%;">
				...
				<?php esc_html_e( 'Logged Out Users', 'wpdx' ); ?>	       
			</label>
			...
			<label for="nav_menu_by_role-for-<?php echo esc_attr( $item->ID ); ?>" style="<?php echo esc_attr( $float ); ?> width: 30%;">
				...
				<?php esc_html_e( 'Everyone', 'wpdx' ); ?>
			</label>

		</fieldset>

		<fieldset class="field-nav_menu_role nav_menu_role_field description-wide" style="margin: 5px 0; <?php echo esc_attr( $hidden ); ?>">
			<legend class="description"><?php esc_html_e( 'Restrict menu item to a minimum role', 'wpdx' ); ?></legend>
			<br />
    ...
    */
    ...
    if ( 'nav-menus.php' === $hook ) {
            wp_enqueue_script( 'cmp-nav-menus', get_template_directory_uri() . '/assets/js/nav-menu-roles.js' , array( 'jquery' ), self::VERSION, true );
        }
    }
...
add_action( 'after_setup_theme', 'nav_menu_roles' );
